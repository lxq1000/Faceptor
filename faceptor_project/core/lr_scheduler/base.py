...
class LinearLRScheduler(_WarmUpLRScheduler):
    ...
    def _get_new_lr(self):
        ...
        step_ratio = (self.last_iter-self.warmup_steps) / (self.max_iter-self.warmup_steps)
        target_lr = self.eta_min + (self.warmup_lr - self.eta_min)*(1 - step_ratio)
        scale = target_lr / self.base_lr
        return [scale*base_lr for base_lr in self.base_lrs]

class HTLTCosineLRScheduler(CosineLRScheduler):
    def __init__(self, optimizer, max_iter, eta_min, base_lr, warmup_lr, warmup_steps, last_iter=-1, freeze_iters=0):
        super(HTLTCosineLRScheduler, self).__init__(optimizer, max_iter, eta_min, base_lr, warmup_lr, warmup_steps, last_iter)
        self.freeze_iters = freeze_iters

    def _get_new_lr(self):
        if self.freeze_iters > 0 and self.last_iter < self.freeze_iters:
            scale = self.warmup_lr / self.base_lr
            return [scale * base_lr for base_lr in self.base_lrs]
        if self.warmup_steps > 0 and (self.last_iter - self.freeze_iters) < self.warmup_steps:
            # first compute relative scale for self.base_lr, then multiply to base_lr
            scale = (((self.last_iter - self.freeze_iters)/self.warmup_steps)*(self.warmup_lr - self.base_lr) + self.base_lr)/self.base_lr
            #print('last_iter: {}, warmup_lr: {}, base_lr: {}, scale: {}'.format(self.last_iter, self.warmup_lr, self.base_lr, scale))
            warmup_lr = [scale * base_lr for base_lr in self.base_lrs]
        else:
            warmup_lr = None
        ...
        step_ratio = (self.last_iter-self.warmup_steps-self.freeze_iters) / (self.max_iter-self.warmup_steps-self.freeze_iters)
        target_lr = self.eta_min + (self.warmup_lr - self.eta_min)*(1 + math.cos(math.pi * step_ratio)) / 2
        scale = target_lr / self.base_lr
        return [scale*base_lr for base_lr in self.base_lrs]

class HTLTDualCosineLRScheduler(HTLTCosineLRScheduler):
    def _get_new_lr(self):
        if self.freeze_iters > 0 and self.last_iter < self.freeze_iters:
            step_ratio = self.last_iter / self.freeze_iters
            target_lr = self.eta_min + (self.warmup_lr - self.eta_min) * (1 + math.cos(math.pi * step_ratio)) / 2
            scale = target_lr / self.base_lr
            return [scale * base_lr for base_lr in self.base_lrs]
