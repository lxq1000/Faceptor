...
def load_farl(model_type, model_file, drop_path_rate) -> VisualTransformer:
    checkpoint = torch.load(model_file, map_location='cpu')
    if model_type == "base":
        model = VisualTransformer(
            input_resolution=224, patch_size=16, width=768, layers=12, heads=12, output_dim=512, drop_path_rate=drop_path_rate)
    elif model_type == "large":
        model = VisualTransformer(
            input_resolution=224, patch_size=16, width=1024, layers=24, heads=16, output_dim=512, drop_path_rate=drop_path_rate)
    elif model_type == "huge":
        model = VisualTransformer(
            input_resolution=224, patch_size=14, width=1280, layers=32, heads=16, output_dim=512, drop_path_rate=drop_path_rate)
    else:
        raise
...
class FaRLVisualFeatures(nn.Module):
    ...
    def __init__(self, model_type: str,
        ...
        self.visual = load_farl(model_type, model_path, drop_path_rate)
    ...
    def forward(self, image: torch.Tensor) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
        ...
        x = x + _resize_pe(self.visual.positional_embedding, new_size=S).to(x.dtype)
