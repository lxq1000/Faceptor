...
class PostSolver(object):
    ...
    def create_model(self):
        ...
        checkpoint = torch.load(self.cfg.model_entry.model_path, 'cpu')
        pretrained_state_dict = checkpoint['state_dict']

        for each in pretrained_state_dict.keys():
            print(each)
        ...
        for name, param in model.named_parameters():
            flag = True
            for each in self.cfg.model_entry.trainable:
                if each in name:
